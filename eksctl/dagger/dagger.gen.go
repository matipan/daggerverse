...
func (r Eksctl) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Cluster   *File
		Container *Container
	}
...
func (r *Eksctl) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Cluster   *File
		Container *Container
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Eksctl":
		...
		case "WithContainer":
			...
			return (*Eksctl).WithContainer(&parent, ctr), nil
		case "Exec":
			var parent Eksctl
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Eksctl).Exec(&parent, ctx, command)
		case "Create":
			var parent Eksctl
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Eksctl).Create(&parent, ctx, flags)
		case "Delete":
			var parent Eksctl
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Eksctl).Delete(&parent, ctx, flags)
		case "Kubeconfig":
			var parent Eksctl
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Eksctl).Kubeconfig(&parent, ctx), nil
		case "":
			var parent Eksctl
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var awsCreds *File
			if inputArgs["awsCreds"] != nil {
				err = json.Unmarshal([]byte(inputArgs["awsCreds"]), &awsCreds)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg awsCreds", err))
				}
			}
			var awsProfile string
			if inputArgs["awsProfile"] != nil {
				err = json.Unmarshal([]byte(inputArgs["awsProfile"]), &awsProfile)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg awsProfile", err))
				}
			}
			var cluster *File
			...
			return New(version, awsCreds, awsProfile, cluster), nil
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Eksctl").
					WithFunction(
						dag.Function("WithContainer",
							dag.TypeDef().WithObject("Eksctl")).
							WithDescription("WithContainer allows you to modify the container used to run eksctl.\nYou should always use the existing `eksctl.Container` and add things on\ntop of it. This is the unsafe alternative to something like accepting a\nfunction as a parameter that modifies the existing container.\nSee https://github.com/dagger/dagger/issues/6213 for more details.").
							WithArg("ctr", dag.TypeDef().WithObject("Container"))).
					WithFunction(
						dag.Function("Exec",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Exec executes the eksctl command.").
							WithArg("command", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)))).
					WithFunction(
						dag.Function("Create",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Create calls `eksctl create` with the cluster config. Additional\nflags can be provided in `exec` form.").
							WithArg("flags", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithFunction(
						dag.Function("Delete",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("DeleteCluster calls `eksctl delete` on the cluster config. Additional\nflags can be provided in `exec` form.").
							WithArg("flags", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithFunction(
						dag.Function("Kubeconfig",
							dag.TypeDef().WithObject("File")).
							WithDescription("Kubeconfig returns the kubeconfig of the cluster. To download it using Dagger's\nCLI you can call `dagger download`.")).
					WithField("Cluster", dag.TypeDef().WithObject("File")).
					WithField("Container", dag.TypeDef().WithObject("Container")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Eksctl")).
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"latest\"")}).
							WithArg("awsCreds", dag.TypeDef().WithObject("File")).
							WithArg("awsProfile", dag.TypeDef().WithKind(StringKind)).
							WithArg("cluster", dag.TypeDef().WithObject("File")))), nil
	default:
